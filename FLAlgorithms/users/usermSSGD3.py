...
class UsermSSGD3(User):
    ...
    def aggregate_parameters2(self, server_param):
        alpha = 1   
        for current_task, server in zip(self.model.parameters(), server_param):
            current_task.data = current_task.data - self.learning_rate * (self.L_k * alpha* (current_task.data.clone() - server.data.clone()))
        #self.clone_model_paramenter(self.model.parameters(), self.local_model)
